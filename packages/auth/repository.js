...
class Repository {
  ...
  hasClientSecret() {
    const chipperText = this.storage.getData(Variables.STORAGE_CLIENT_SECRET);
    if (chipperText == '' || chipperText == null) {
      return false;
    } else {
      const clientSecret = this.crypto.decrypt(chipperText);
      return clientSecret != '';
    }
  }

  resetStorage() {
    this.storage.setData(Variables.STORAGE_LOGIN_SECRET, '');
    this.storage.setData(Variables.STORAGE_CLIENT_SECRET, '');
    this.storage.setData('user', '');
  }
  ...
  setupLoginToken(tokenLogin) {
    const chipperTextLoginToken = this.crypto.encrypt(tokenLogin);
    this.storage.setData(Variables.STORAGE_LOGIN_SECRET, chipperTextLoginToken);
  }

  setupClientSecret(clientSecret) {
    const chiperText = this.crypto.encrypt(clientSecret);
    this.storage.setData(Variables.STORAGE_CLIENT_SECRET, chiperText);
  }

  getDeviceId() {
    const chipperText = this.storage.getData(Variables.STORAGE_DEVICE_ID);
    const deviceId = this.crypto.decrypt(chipperText);
    return deviceId;
  }
  ...
  serviceGenerateClientSecret(callback) {
    ...
    this.service.setHeader('client-secret', process.env.WEB_API_KEY);
  ...
  storeLogin(params, callback) {
    params['device_id'] = this.auth.getDeviceId();
    setTimeout(() => {
