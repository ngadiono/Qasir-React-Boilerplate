...
class FormItemEmail extends React.Component {
    ...
    render(){
        const { form, defaultValue, fieldName, label, validateCustom, message, required, autoComplete, placeholder} = this.props
        ...
        if (validateCustom) {
              return (
                <FormItem label={label} validateStatus={validateCustom ? validateCustom.title : ""} help={validateCustom ? validateCustom.message : ""}>
                    ...
                    })(<Input placeholder={placeholder} autocomplete={autoComplete} size="default" onChange={this.handleValuechange}/>)}
                </FormItem>
                )
        } else {
          return (
